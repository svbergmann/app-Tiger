...
=== FM07 How can I use fallback values in a Scenario Outline?

Use the syntax `${configuration.key\|fallbackValue}` in an Examples table.
The backslash escapes the pipe because `|` is also the Gherkin table-column separator.

[source,gherkin]
----
Scenario Outline: Register multiple users
  Given TGR send GET request to "http://demo/service/register" with:
    | username | password   |
    | <name>   | <password> |
  ...
  And TGR current response with attribute "$.body.name" matches "<name>"

  Examples:
    | name                               | password              |
    | ${demo.user.name}                  | ${demo.user.password} |
    | ${demo.user.noName\|defaultUser} | xys                   |
----

For the first example, Tiger resolves `demo.user.name` and `demo.user.password` from the configuration.
For the second example, Tiger tries to resolve `demo.user.noName`.
If no value is available for this key, Tiger uses `defaultUser` as the username.
