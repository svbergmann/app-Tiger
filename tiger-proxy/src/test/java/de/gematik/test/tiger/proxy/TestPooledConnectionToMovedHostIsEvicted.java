...
class TestPooledConnectionToMovedHostIsEvicted extends AbstractTigerProxyTest {
  ...
  @Test
  void aPooledConnectionToAMovedHostShouldBeDroppedAndTheNextRequestShouldSettleOnTheNewOne()
      throws Exception {
            ...
            .proxyRoutes(
                List.of(
                    TigerConfigurationRoute.builder()
                        .from("/")
                        .to(List.of("http://" + MOVING_HOST + ":" + sharedPort))
                        .build()))
            .build());

    val clientSharingOneConnectionAcrossRequests =
        HttpClient.newBuilder().connectTimeout(Duration.ofSeconds(5)).build();

    assertThat(whoAnsweredOn(clientSharingOneConnectionAcrossRequests))
        .as("the first request opens the pooled connection, to where the host resolves now")
        .isEqualTo("first");
    ...
    assertThat(whoAnsweredOn(clientSharingOneConnectionAcrossRequests))
        .as(
            "the pooled connection points at the backend the host has just left, so it has to be"
                + " dropped rather than handed back")
        .isEqualTo("second");
    assertThat(whoAnsweredOn(clientSharingOneConnectionAcrossRequests))
        .as(
            "and the replacement has to be kept: it points where the host now is, so evicting again"
                + " here would mean pooling never recovers from a move")
        .isEqualTo("second");
  }
